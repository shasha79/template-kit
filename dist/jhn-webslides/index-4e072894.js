const NAMESPACE = 'jhn-webslides';
const BUILD = /* jhn-webslides */ {
  allRenderFn: true,
  ...
  if (BUILD.vdomText && newVNode.$text$ !== null) {
    ...
    elm = newVNode.$elm$ = BUILD.svg
      ? doc.createElementNS(
          ...
          BUILD.slotRelocation && newVNode.$flags$ & 2 /* isSlotFallback */ ? 'slot-fb' : newVNode.$tag$
        )
...
const slotReferenceDebugNode = (slotVNode) =>
  doc.createComment(`<slot${slotVNode.$name$ ? ' name="' + slotVNode.$name$ + '"' : ''}> (host=${hostTagName.toLowerCase()})`);
...
const addHydratedFlag = (elm) =>
  BUILD.hydratedClass ? elm.classList.add('hydrated') : BUILD.hydratedAttribute ? elm.setAttribute('hydrated', '') : undefined;
  ...
  if (BUILD.member && cmpMeta.$members$) {
    ...
    members.map(([memberName, [memberFlags]]) => {
      if (
        ...
        (memberFlags & 31 /* Prop */ || ((!BUILD.lazyLoad || flags & 2) /* proxyState */ && memberFlags & 32) /* State */)
      ) {
        // proxyComponent - prop
